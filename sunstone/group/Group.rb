...
class Group
    ...
    def create(name, hash_create)
        @utils.navigate(@general_tag, @resource_tag)
        if !@utils.check_exists(2, name, @datatable)
            @utils.navigate_create(@general_tag, @resource_tag)
            @sunstone_test.get_element_by_id("name").send_keys "#{name}"
            if hash_create[:views]
                @sunstone_test.get_element_by_id("resource_views-label").click

                if hash_create[:views][:dafault_user]
                    options = $driver.find_elements(:xpath, "//select[@id='user_view_default']//option")
                    options.each{ |opt| opt.click if opt.attribute("value") == hash_create[:views][:dafault_user]}
                end
                if hash_create[:views][:dafault_admin]
                    options = $driver.find_elements(:xpath, "//select[@id='admin_view_default']//option")
                    options.each{ |opt| opt.click if opt.attribute("value") == hash_create[:views][:dafault_admin]}
                end

                if hash_create[:views][:all]
                    hash_create[:views][:all].each{ |view|
